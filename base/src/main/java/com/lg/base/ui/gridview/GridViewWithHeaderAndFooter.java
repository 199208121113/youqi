 ...
 * See {@link com.ireadercity.widget.GridViewWithHeaderAndFooter#addHeaderView(View, Object, boolean)}
 * See {@link com.ireadercity.widget.GridViewWithHeaderAndFooter#addFooterView(View, Object, boolean)}
 */
public class GridViewWithHeaderAndFooter extends GridView implements AbsListView.OnScrollListener{
    ...
    private static class HeaderViewGridAdapter implements WrapperListAdapter, Filterable {
        ...
        public boolean isEnabled(int position) {
            ...
            return footerPosition % mNumColumns == 0 && mFooterViewInfos.size() > 0
    ...
    public void onScrollStateChanged(AbsListView view, int scrollState) {
        ...
        if (scrollState == OnScrollListener.SCROLL_STATE_IDLE) {
            int headerItemCount = getHeaderViewCount()*getNumColumnsCompatible();
            if(mCurrentIndex<headerItemCount){//说明当前的Header处于显示状态
                View headerView = getChildAt(0);
                if(headerView == null)
                    return;
                int scrollSize = getFirstVisiblePosition()*headerView.getHeight()-headerView.getTop();
                if(scrollSize != 0 && scrollSize < headerView.getHeight()){
                   setSelection(headerItemCount);
                }
            }
        }
    ...
    public void onScroll(AbsListView view, int firstVisibleItem, int visibleItemCount, int totalItemCount) {
        mCurrentIndex = firstVisibleItem;
        int headerItemCount = getHeaderViewCount()*getNumColumnsCompatible();
        if(mCurrentIndex > mLastIndex){ //向下滑动（手势向上）

        }else if(mCurrentIndex < mLastIndex){ //向上滚动
            if(mCurrentIndex < headerItemCount ){
                if( mLastIndex > headerItemCount) {
                    stopScrollForSelf(this);
                    setSelection(headerItemCount);
                }else if(mLastIndex == headerItemCount){
                }
            }
        }
    }
    ...
    public static void stopScrollForSelf(GridView list) {
